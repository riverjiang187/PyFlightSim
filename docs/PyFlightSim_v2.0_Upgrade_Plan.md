...
4. [P1 阶段技术债务清单 (Technical Debt)](#4-p1-阶段技术债务清单-technical-debt)
   - [TD-1: P1-1 遗留——Estimator 滤波算法硬编码](#td-1-p1-1-遗留estimator-滤波算法硬编码)
...
### P1-1: 传感器高保真误差、随机过程噪声模型与独立状态估计器 (State Estimator)
...
* **技术设计与数学模型**：
  ...
  4. **独立状态估计器 (Estimator)**：
     * 在独立的 `estimator.py` 中实现滤波算法（如互补滤波/LPF），专门接收带噪传感器输入。
     * 设计显式的状态获取接口，并提供状态开关：例如 `use_filtered=True/False`，以支持按需输出平滑滤波数据或原始带噪数据。
  5. **架构解耦与接口规范**：
     * 传感器仅向 `Estimator` 输送原始数据。规范要求未来的所有新下游组件必须全面通过显式调用 `Estimator` 获取状态，严禁直连传感器。
  6. **配置文件架构与软硬分离规范**：
     * **硬件属性 (`configs/aircraft.yaml`)**：新增 `sensors` 节点。将不同机型的传感器精度（如 IMU 的游走方差、GPS 的定位误差及延迟参数）作为航空电子硬件属性绑定至飞机。
     * **仿真控制 (`configs/simulation.yaml`)**：新增 `environment -> sensors -> enable_noise` 开关。用于一键启停所有传感器的物理噪声，实现真值与带噪状态的快速切换。
* **验收标准 (Acceptance Criteria)**：
  1. 长时间（300s）静止工况下，采集 IMU 输出序列，样本方差与配置设定的白噪声谱密度吻合度 $\ge 95\%$，零偏具备明显低频漂移特征。
  2. GPS 在 100Hz 物理主循环下，输出读数呈现阶梯状更新（对应配置的 10Hz）。
  3. **滤波器隔离测试（单元测试）**：向 `Estimator` 注入合成的带噪信号。当 `use_filtered=False` 时，输出严格等于原始输入；切换至 `use_filtered=True` 时，输出信号能有效滤除高频噪声，且动态响应的相位滞后处于理论允许范围内。
...
## 4. P1 阶段技术债务清单 (Technical Debt)
*以下条目记录 P1 阶段实现过程中已发现但未在当次迭代中完整解决的设计遗留问题（Technical Debt）。这些问题不是新功能，而是对已完成任务中存在的工程缺陷的补救，需在对应 P1 任务**正式收尾前**完成修复。*

---

### TD-1: P1-1 遗留——Estimator 滤波算法硬编码

* **来源任务**：P1-1（传感器高保真误差与随机过程噪声模型）
* **涉及文件**：
  * `modules/sensors/estimator.py`
  * `configs/aircraft.yaml`
* **问题描述**：
  P1-1 实现 `Estimator` 时存在两处硬编码遗留：
  1. **算法类型固定**：当前仅实现了简单指数移动平均（EMA/LPF）一种滤波算法，写死在类内部，无法切换。事实上，不同机型对估计器的需求在算法层面存在本质差异，而不仅仅是参数不同：
     - **慢速通航飞机（如 C172）**：控制带宽低（< 2Hz），简单 LPF 完全够用，实现简单、计算开销极低；
     - **高机动固定翼 / 小型无人机**：控制带宽可达 20~50Hz，需要**互补滤波（Complementary Filter）**，将陀螺积分（高频准确）与加速度计修正（低频准确）按截止频率融合，才能同时保证姿态动态响应速度和静态精度；
     - **科研/高保真验证平台**：需要**扩展卡尔曼滤波（EKF）**，多传感器最优融合，提供协方差估计，但计算开销最高。
  2. **算法参数硬编码**：LPF 时间常数 `tau = 0.1` 写死为字面量，没有从 `aircraft.yaml` 的 `sensors` 节点读取，导致 P2-5 多机型参数库建设时所有机型估计器行为完全一致，失去物理区分意义。

* **技术设计与修复方案（策略模式）**：
  采用**策略模式（Strategy Pattern）**，将滤波算法解耦为可插拔的独立实现：

  1. **定义抽象基类 `BaseFilter`**（`modules/sensors/filters/base.py`）：
     ```python
     class BaseFilter:
         def update(self, raw_rates, raw_accel, dt) -> tuple:
  ...
  2. **实现三套具体算法**：
     | 文件 | 算法 | 适用机型 | 关键参数 |
     | :--- | :--- | :--- | :--- |
     | `filters/lpf.py` | 指数低通滤波（EMA） | 慢速通航（C172 等） | `tau`（时间常数，秒） |
     | `filters/complementary.py` | 互补滤波 | 高机动固定翼 / 小型 UAV | `tau`（截止时间常数） |
     | `filters/ekf.py` | 扩展卡尔曼滤波（EKF） | 科研高保真平台 | `Q`（过程噪声矩阵）、`R`（观测噪声矩阵） |

  3. **`configs/aircraft.yaml` 新增 `estimator` 节点**（归属在已有 `sensors` 节点下）：
     ```yaml
     sensors:
       # ... (已有的 imu / gps / adc 节点保持不变)
       estimator:
         filter_type: "lpf"        # 可选: "lpf" | "complementary" | "ekf"
         # filter_type 对应的参数块 (只解析与 filter_type 匹配的块)
         lpf:
           imu_lpf_tau: 0.1        # 截止时间常数 (s); f_c ≈ 1/(2π·tau)
         complementary:
           tau: 0.05               # 互补滤波交叉截止时间常数 (s)
         ekf:
           q_gyro: 0.001           # 过程噪声 - 陀螺 (rad²/s²)
           q_accel: 0.01           # 过程噪声 - 加速度计 (m²/s⁴)
           r_accel: 0.05           # 观测噪声 - 加速度计 (m²/s⁴)
     ```

  4. **`Estimator` 工厂化构造**：
     * 构造函数读取 `filter_type` 字段，用工厂函数实例化对应 `BaseFilter` 子类；
     * `update()` 方法统一调用 `self.filter.update(...)`，对外接口完全不变。

* **验收标准 (Acceptance Criteria)**：
  1. **算法切换验证**：仅修改 `aircraft.yaml` 中的 `filter_type` 字段（无需改动任何 Python 代码），三种算法均能正常加载运行；
  2. **差异化行为验证**：向三套 Estimator 注入相同的合成带噪 IMU 信号，验证三者的输出信号在频域特征（截止频率）上存在统计显著差异；
  3. **无破坏性回归**：默认配置 `filter_type: "lpf"` + `imu_lpf_tau: 0.1` 下，输出结果与旧版 P1-1 完全一致（RMSE < $10^{-10}$）。
...
| :--- | :--- | :---: | :--- | :--- | :---: | :---: | :---: |
| **P1-1** | 传感器高保真误差噪声 | **P1** | 高斯白噪声、零偏游走与延迟缓冲 | `imu.py`, `gps.py` 等 | v1.0.2 基线 | 低 | ✅ 完成 |
...
| — | — | — | — | — | — | — | — |
| **TD-1** | Estimator 滤波算法硬编码 | **债务** | 策略模式重构，支持 LPF/互补/EKF 可配置切换 | `estimator.py`, `filters/`, `aircraft.yaml` | P1-1 | 低 | ⏳ 待办 |
